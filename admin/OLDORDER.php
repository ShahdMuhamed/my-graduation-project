...
if (isset($_GET['logout'])) {
    session_unset();
    session_destroy();
    header("location:/projects/carix/customer/Admin-Login.php");
} 
...
if(isset($_POST['change'])){
    ...
    header("location:/projects/carix/admin/vieworder.php");
}
            ...
            <div class="collapse navbar-collapse centerNav" id="navbarSupportedContent">
                ...
                <ul class="navbar-nav mx-auto mb-2 mb-lg-0 gap-4">
                    <li class="nav-item dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <!-- <li><a class="dropdown-item" href="./Add-Order.php">edit Orders</a></li> -->
        ...
        <div class="container view">
            ...
            <div class="table-responsive">
                <table class="appointment">
                    ...
                    <?php foreach($run_select as $order){
                        $email = substr($order['email'], 0, strpos($order['email'], "@"));
                        $id = $order['o_id'];
                        $selec†_service = "SELECT * FROM `order_details`
                        INNER JOIN `services`
                        ON `order_details`.`service_id` = `services`.`s_id`
                        WHERE `order_details`.`order_id` = $id" ; 
                        $run_select_service = mysqli_query($connect , $selec†_service);
                        $select_product = "SELECT * FROM `order_details`
                        INNER JOIN `products`

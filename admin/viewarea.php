...
if(isset($_POST['search'])){
    ...
    $select="SELECT * FROM `areas` WHERE `name` like '%$text%' or `city` like '%$text%'" ;
    $run_select=mysqli_query($connect,$select);
    }elseif(isset($_POST['sort'])){
    $value=$_POST['value'];
    if($value==1)
    {
        $select="SELECT * FROM `areas` ORDER BY `id` ASC";
        $run_select=mysqli_query($connect,$select);
    }else{
        $select="SELECT * FROM `areas` ORDER BY `id`  DESC";
        $run_select=mysqli_query($connect,$select);
    }
...
if(isset($_GET['delete'])){
    ...
    header("location:/projects/carix/admin/viewarea.php");
}

//add icon 
if(isset($_POST['add'])){
    header("location:/projects/carix/admin/Add-Area.php");
}
//logout
    ...
    <section class="mainBg py-5">
        <div class="container view">
            <section class="filterBar justify-content-between d-flex align-items-center my-3">
                <div class="addIcon">
                    <form method="POST">
                        <button name="add" type="submit" class="px-3 rounded align-items-center d-flex fs-5"><span
            ...
            <table class="area">
                ...
                <tr>
                    <td class="serviceName"><?php echo $row['name'] ?></td>
                    <td class="text-center"><?php echo $row['city'] ?></td>
                    <td class="update text-center fs-5"><a href="Add-Area.php?edit=<?php echo $row['id']; ?>"><i class="fa-solid fa-rotate"></i></a></td>
                    <td class="delete text-center fs-5"><a href="viewarea.php?delete=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this area?')"><i class="fa-solid fa-trash"></i></a></td>
                </tr>
              <?php } ?>
            </table>
        </div>

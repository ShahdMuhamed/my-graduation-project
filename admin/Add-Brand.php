...
if(isset($_GET['update'])){
    ...
    WHERE `id` = $brand_id";
    $run_select_brand = mysqli_query($connect , $select_brand);
    $array = mysqli_fetch_array($run_select_brand);
}
...
if(isset($_POST['update'])){
    ...
    if (!empty($_FILES['logo']['name'])) {
        $upload = move_uploaded_file($_FILES['logo']['tmp_name'], "../images/Home/CarBrands/" . $_FILES['logo']['name']);
    }
    if ($run_update) {
...
if(isset($_POST['add'])){
    $name = $_POST['name'];
    $logo = $_FILES['logo']['name'];
    $insert = "INSERT INTO `brands` VALUES(NULL , '$name' , '$logo')";
                ...
                <div class="inner">
                    ...
                    <?php if($mode == 'add'){?>
                        ...
                        <div class="form-holder">
                            <input type="text" placeholder="Brand Name" class="form-control" style="font-size: 15px;" name="name" value="<?php if ($mode == 'update'){ echo $array['name']; } ?> ">
                        </div>
                        <div class="form-holder">

...
if(isset($_POST['search'])){
    ...
    $select="SELECT `wallets`.`id` as `wallet_id` , `user_id`,`code`,`email`,`cash`FROM `wallets`
    JOIN `users` ON `wallets`.`user_id`=`users`.`id`
    WHERE `email` like '%$text%' or `code` like '%$text%' or `cash` like '%$text%'" ;
    ...
    {
        $select="SELECT `wallets`.`id` as `wallet_id` , `user_id`,`code`,`email`,`cash`FROM `wallets`
        JOIN `users` ON `wallets`.`user_id`=`users`.`id` ORDER BY `wallet_id` ASC";
        $run_select=mysqli_query($connect,$select);
    }else{
        $select="SELECT `wallets`.`id` as `wallet_id` , `user_id`,`code`,`email`,`cash`FROM `wallets`
        JOIN `users` ON `wallets`.`user_id`=`users`.`id` ORDER BY `wallet_id` DESC";
        $run_select=mysqli_query($connect,$select);
    }
...
if(isset($_GET['delete'])){
    $id=$_GET['delete'];
    $delete="DELETE FROM `wallets` WHERE `id`=$id";
    $run_delete=mysqli_query($connect,$delete);
    header("location:/projects/carix/admin/viewwallet.php");
}
    ...
    <section class="mainBg py-5">
        <div class="container view">
            <section class="filterBar justify-content-between d-flex align-items-center my-3">
                ...
                <div class="search-ui d-flex align-items-center">
                    <div class="search-container">
                        <form method="POST">
                            <input type="text" placeholder="Search by name..." name="text">
            ...
            <table class="area">
                ...
                <tr>
                    <td class="p-0 text-center"><?php echo $row['email'] ?></td>
                    <td class="text-center"><?php echo $row['code'] ?></td>
                    <td class="text-center"><?php echo $row['cash'] ?></td>
                    <!-- <td class="update text-center fs-5"><a href=""><i class="fa-solid fa-rotate"></i></a></td> -->
                    <td class="delete text-center fs-5"><a href="viewwallet.php?delete=<?php echo $row['wallet_id']; ?>" onclick="return confirm('Are you sure you want to delete this wallet?')"><i class="fa-solid fa-trash"></i></a></td>
                </tr>
                <?php } ?>
            </table>
        </div>

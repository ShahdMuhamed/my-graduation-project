...
if(isset($_POST['search'])){
    ...
    $select="SELECT * FROM `appointments` WHERE `name` like '%$text%' or `phone` like '%$text%' 
    or `reason` like '%$text%' or `car_details` like '%$text%'
    or `date` like '%$text%' or `time` like '%$text%' or `status` like '%$text%' or `appointment_address` like '%$text%'" ;
    $run_select=mysqli_query($connect,$select);
    }elseif(isset($_POST['sort'])){
    $value=$_POST['value'];
    if($value==1)
    {
        $select="SELECT * FROM `appointments` ORDER BY `id` ASC";
        $run_select=mysqli_query($connect,$select);
    }else{
    ...
    <section class="mainBg py-5">
        <div class="container view">
            <section class="filterBar justify-content-between d-flex align-items-center my-3">
                ...
                <div class="search-ui d-flex align-items-center">
                    <div class="search-container">
                        <form method="POST">
                            <input type="text" placeholder="Search by user name..." name="text">
            ...
            <div class="table-responsive">
                <table class="appointment appTable">
                    <tr class="table-header">
                        <th>User Name</th>
                        <th class="ps-0 text-center">Phone</th>
                        <th class="ps-0 text-center">Reason</th>
                        <th class="ps-0 text-center">Car Details</th>
                    ...
                    <?php foreach($run_select as $row){ $status=$row['status'];?>
                    <tr>
                        <td><?php echo $row['name'] ?></td>
                        <td class="p-0 text-center"><?php echo $row['phone'] ?></td>
                        <td class="p-0 text-center reason"><?php echo $row['reason'] ?></td>
                        <td class="p-0 text-center reason"><?php echo $row['car_details'] ?></td>
                        <td class="p-0 text-center reason"><?php echo $row['appointment_address'] ?></td>
                        <td class="p-0 text-center"><?php echo $row['date'] ?></td>
                        <td class="p-0 text-center"><?php echo $row['time'] ?></td>
                        <?php if($status=='accepted'){?>
                        <td class="p-0 text-center text-success fw-bold"><?php echo $row['status'] ?></td>
                        <?php }elseif($status=='cancelled'){?>
                        <td class="p-0 text-center text-danger fw-bold"><?php echo $row['status'] ?></td>
                        <?php }else{?>
                        <td class="p-0 text-center text-primary fw-bold"><?php echo $row['status'] ?></td>
                        <?php } ?>
                        <form method="POST">
                        <td class="py-3 px-2 border-0 text-center status">
                            <select name="status" id="viewTable" aria-label="Sort By" class="form-select">
                                <option>Choose</option>
                                <option value="1" >accepted</option>
                                <option value="2" >pending</option>
                                <option value="3" >cancelled</option>
                            </select> <br>
                            <button value="<?php echo $row['id']; ?>" name="choose" type="submit" class="px-3 py-1 rounded align-items-center d-flex">Change</button>
                        </td>
                        </form>
                        <td class="delete text-center fs-5"><a href="viewappointment.php?delete=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure you want to delete this appointment?')"><i class="fa-solid fa-trash"></i></a></td>
                    </tr>
                   <?php } ?>
                </table>
            </div>

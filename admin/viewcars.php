...
if(isset($_POST['search'])){
    ...
    $select="SELECT `car_id`,`model_id`,`user_id`,`email`,`cc`,`year`,`details`,`models`.`name` as `model_name` ,
    `brand_id`,`brands`.`name` as `brand_name` FROM `cars`
    JOIN `users` ON `cars`.`user_id`=`users`.`id` 
    JOIN `models` ON `cars`.`model_id`=`models`.`id` 
    JOIN `brands` ON `models`.`brand_id`=`brands`.`id` WHERE `brands`.`name` like '%$text%' or `models`.`name` like '%$text%'
    or `cars`.`year` like '%$text%' or `cars`.`cc` like '%$text%' or `users`.`email` like '%$text%' " ;
    ...
    if($value==1)
    {
        ...
        JOIN `users` ON `cars`.`user_id`=`users`.`id` 
        JOIN `models` ON `cars`.`model_id`=`models`.`id` 
        JOIN `brands` ON `models`.`brand_id`=`brands`.`id` ORDER BY `car_id` ASC";
        $run_select=mysqli_query($connect,$select);
    }else{
        $select="SELECT `car_id`,`model_id`,`user_id`,`email`,`cc`,`year`,`details`,`models`.`name` as `model_name` ,
        `brand_id`,`brands`.`name` as `brand_name` FROM `cars`
        JOIN `users` ON `cars`.`user_id`=`users`.`id` 
        JOIN `models` ON `cars`.`model_id`=`models`.`id` 
        JOIN `brands` ON `models`.`brand_id`=`brands`.`id` ORDER BY `car_id` DESC";
        $run_select=mysqli_query($connect,$select);
    }
...
if(isset($_GET['delete'])){
    ...
    header("location:/projects/carix/admin/viewcars.php");
}
        ...
        <div class="container view">
            <section class="filterBar justify-content-between d-flex align-items-center my-3">
                ...
                <div class="search-ui d-flex align-items-center">
                    <div class="search-container">
                        <form method="POST">
                            <input type="text" name="text" placeholder="Search by car model..." >
            ...
            <table class="">
                <tr class="table-header">
                    ...
                    <th class="ps-0 text-center">Year</th>
                    <th class="ps-0 text-center">Details</th>
                ...
                <tr>
                    ...
                    <td class="p-0 text-center"><?php echo $row['cc']?></td>
                    <td class="p-0 text-center"><?php echo $row['year']?></td>
                    <td class="p-0 text-center"><?php echo $row['details']?></td>
                    <!-- <td class="update text-center fs-5"><a href=""><i class="fa-solid fa-rotate"></i></a></td> -->
                    <td class="delete text-center fs-5"><a href="viewcars.php?delete=<?php echo $row['car_id']; ?>" onclick="return confirm('Are you sure you want to delete this car?')"><i class="fa-solid fa-trash"></i></a></td>
                </tr>
               <?php } ?>
            </table>
        </div>
